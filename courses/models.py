...
class DegreeIndexPage(Page):
    ...
    degree_type = models.CharField(
        max_length=3,
        choices=DEGREE_INDEX_TYPE_CHOICES,
        default='HE'
    )
    ...
    ready_title = models.CharField(max_length=255, null=True, blank=True)

    ready_links = StreamField(
        [
    ...
    team_title = models.CharField(max_length=255, null=True, blank=True)

    elicos_team = StreamField(
        [
            ("department", blocks.StructBlock([
                ("department_title", blocks.CharBlock()),
                ("team_members", blocks.ListBlock(
                    blocks.StructBlock([
                        ("name", blocks.CharBlock()),
                        ("position", blocks.RichTextBlock()),
    ...
    content_panels = Page.content_panels + [
        ...
        # Ready to Study
        FieldPanel('ready_title'),
        FieldPanel('ready_links'),

        # Team
        FieldPanel('team_title'),
        FieldPanel('elicos_team'),
    ]
...
class CourseUnitBlock(blocks.StructBlock):
    display_as_dropdown = blocks.BooleanBlock(
        default=True,
        required=False,
        help_text="If false, only course title will be displayed as lists"
    )
...
class HigherEducationCoursePage(Page):
    ...
    course_type = models.CharField(
        max_length=3,
        choices=HE_DEGREE_TYPE_CHOICES,
        default='UG'
    )
    ...
    core_units_block = HTMLField(
        null=True,
        blank=True,
        help_text="Use when only unit titles are needed"
    )
